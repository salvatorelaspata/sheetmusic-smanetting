...
import { durationNameIt, PALETTE_DURATIONS } from '../../core/durations'
...
function makeQuestion(): Question {
  ...
  const correct = durationNameIt(base)
  const distractors = sample(
    BASES.filter((b) => b !== base),
    3,
  ).map(durationNameIt)
  const options = shuffle([correct, ...distractors])
  return { base, isRest, options, correctIndex: options.indexOf(correct) }
}
